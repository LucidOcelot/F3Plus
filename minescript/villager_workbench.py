...
The explorer paints only visible trade cards and loads installed-version trade JSON in
the background. A complete labeled reference dataset is available immediately so
opening or typing in Villagers never waits for JAR parsing.
"""
...
from PySide6.QtWidgets import (
    ...
    QProgressBar, QPushButton, QSpinBox, QSplitter, QStyle, QStyledItemDelegate,
...
from .villagers import (
    BASELINE_SOURCE, LEVEL_NAMES, PROFESSIONS, baseline_trades, load_for_version,
    search as trade_search, trade_direction, trade_key,
)
...
class TradeModel(QAbstractListModel):
    ...
    def data(self, index, role=Qt.DisplayRole):
        ...
        if role == Qt.ToolTipRole:
            return f"{trade.wants}{' + ' + trade.additional_wants if trade.additional_wants else ''} → {trade.gives}"
        return None


class TradeDelegate(QStyledItemDelegate):
    """Paint a transaction card without allocating widgets for every trade."""

    def __init__(self, assets: AssetProvider, parent=None):
        super().__init__(parent); self.assets = assets

    def sizeHint(self, option, index): return QSize(max(360, option.rect.width()), 76)
    ...
    def paint(self, painter: QPainter, option, index):
        ...
        painter.drawText(QRect(rect.left() + 48, rect.top() + 7, 420, 20), Qt.AlignLeft | Qt.AlignVCenter, f"{trade.profession.title()} • {LEVEL_NAMES.get(trade.level, trade.level)}")
        ...
        painter.drawText(QRect(rect.right() - 135, rect.top() + 7, 125, 20), Qt.AlignRight | Qt.AlignVCenter, f"max {trade.max_uses if trade.max_uses is not None else '?'} • XP {trade.xp if trade.xp is not None else '?'}")
        painter.restore()
...
class VillagerExplorerDialog(QDialog):
    def __init__(self, owner, profession: str | None = None, mode: str = "Trade Browser"):
        ...
        self.owner = owner; self.mode = mode; self._load_job = None
        self.trades = complete_reference(list(baseline_trades())); self.source = REFERENCE_SOURCE
        self.jar = MinecraftJarData(owner.settings.minecraft_version); self.assets = AssetProvider(self.jar)
        self.compare_keys: list[str] = []; self.rows = []; self._portrait_cache = {}
        ...
        titles = QVBoxLayout(); title = QLabel("Villager Explorer"); title.setObjectName("WorkspaceTitle"); titles.addWidget(title)
        intro = QLabel("Browse every loaded offer by profession and level, search by item/enchantment, compare transactions, and plan repeated use. Only visible trade cards are rendered, so large trade datasets remain responsive.")
        intro.setWordWrap(True); intro.setObjectName("Muted"); titles.addWidget(intro)
        self.source_label = QLabel(); self.source_label.setWordWrap(True); self.source_label.setObjectName("Muted"); titles.addWidget(self.source_label)
        load_row = QHBoxLayout(); self.loading_label = QLabel("Checking installed Minecraft trade data…"); self.loading_label.setObjectName("Muted"); load_row.addWidget(self.loading_label)
        self.loading = QProgressBar(); self.loading.setRange(0, 0); self.loading.setTextVisible(False); self.loading.setMaximumWidth(220); load_row.addWidget(self.loading); load_row.addStretch(); titles.addLayout(load_row); hh.addLayout(titles, 1)
        self.reload_button = QPushButton("Reload installed trade data"); self.reload_button.clicked.connect(self.reload); hh.addWidget(self.reload_button); root.addWidget(hero)
        ...
        self._select_profession(target if target in PROFESSIONS else "")
        self.refresh(); self._begin_load()

    def _build_browser(self, tabs):
        page = QWidget(); root = QVBoxLayout(page); root.setContentsMargins(8, 8, 8, 8); root.setSpacing(8)
        filters = QHBoxLayout(); self.level = QComboBox(); self.level.addItem("All levels"); self.level.addItems([f"{i} — {LEVEL_NAMES[i]}" for i in range(1, 6)])
        self.direction = QComboBox(); self.direction.addItems(["All directions", "Villager sells to you", "Villager buys from you", "Exchange"])
        self.query = QLineEdit(); self.query.setPlaceholderText("Search item, enchantment, profession or offer…"); self.query.setClearButtonEnabled(True)
        self.uses = QSpinBox(); self.uses.setRange(1, 9999); self.uses.setValue(64 if self.mode == "Trade Cycle Calculator" else 12); self.uses.setPrefix("Plan "); self.uses.setSuffix(" uses")
        ...
        professions = QFrame(); professions.setObjectName("ExplorerRail"); pv = QVBoxLayout(professions); label = QLabel("PROFESSIONS"); label.setObjectName("DeckLabel"); pv.addWidget(label)
        self.profession_list = QListWidget(); self.profession_list.setIconSize(QSize(36, 36)); pv.addWidget(self.profession_list, 1); split.addWidget(professions); self._populate_professions(False)

        trades = QFrame(); trades.setObjectName("ExplorerTrades"); tv = QVBoxLayout(trades); head = QHBoxLayout(); trade_label = QLabel("TRADES"); trade_label.setObjectName("DeckLabel"); head.addWidget(trade_label); head.addStretch(); self.trade_count = QLabel(); self.trade_count.setObjectName("Muted"); head.addWidget(self.trade_count); tv.addLayout(head)
        ...
        transaction = QFrame(); transaction.setObjectName("TradeTransaction"); tr = QHBoxLayout(transaction); self.want_icon = QLabel(); self.want_icon.setFixedSize(44, 44); self.want_text = QLabel(); self.want_text.setWordWrap(True); arrow = QLabel("→"); arrow.setObjectName("TradeArrow"); self.give_icon = QLabel(); self.give_icon.setFixedSize(44, 44); self.give_text = QLabel(); self.give_text.setWordWrap(True); tr.addWidget(self.want_icon); tr.addWidget(self.want_text, 1); tr.addWidget(arrow); tr.addWidget(self.give_icon); tr.addWidget(self.give_text, 1); dv.addWidget(transaction)
        ...
        actions = QHBoxLayout(); self.favorite = QPushButton("☆ Favorite"); self.compare = QPushButton("Add to compare"); actions.addWidget(self.favorite); actions.addWidget(self.compare); dv.addLayout(actions)
        compare_label = QLabel("COMPARE"); compare_label.setObjectName("DeckLabel"); dv.addWidget(compare_label); self.compare_list = QListWidget(); self.compare_list.setMaximumHeight(170); dv.addWidget(self.compare_list); clear = QPushButton("Clear comparison"); clear.clicked.connect(self.clear_compare); dv.addWidget(clear); split.addWidget(detail); split.setSizes([210, 760, 430])
    ...
    def _build_planning(self, tabs):
        page = QWidget(); root = QVBoxLayout(page); root.setContentsMargins(12, 12, 12, 12)
        root.addWidget(ExplanationCard("Villager planning", "Curing, halls, workstation counts and breeding food stay beside the trade explorer. Each helper opens an explained parameter/result panel rather than exposing trade/NBT internals."))
    ...
    def _populate_professions(self, portraits=True):
        selected = self.selected_profession() if hasattr(self, "profession_list") and self.profession_list.count() else ""
        ...
        for profession in PROFESSIONS:
            pix = self._portrait_pixmap(profession) if portraits else QPixmap(); item = QListWidgetItem(QIcon(pix) if not pix.isNull() else QIcon(), profession.title()); item.setData(Qt.UserRole, profession); self.profession_list.addItem(item)
        self.profession_list.blockSignals(False); self._select_profession(selected)

    def _select_profession(self, profession: str):
        wanted = str(profession or "")
        for row in range(self.profession_list.count()):
            if str(self.profession_list.item(row).data(Qt.UserRole) or "") == wanted:
                self.profession_list.setCurrentRow(row); return
        if self.profession_list.count(): self.profession_list.setCurrentRow(0)
    ...
    def _begin_load(self):
        if self._load_job is not None: return
        self.reload_button.setEnabled(False); self.loading.show(); self.loading_label.setText("Reference trades are usable now • checking installed-version trade data in the background…")
        version = self.owner.settings.minecraft_version
        self._load_job = start_job(lambda: _load_trade_data(version), finished=self._loaded, failed=self._load_failed, cancelled=self._load_cancelled)

    def _loaded(self, payload):
        self._load_job = None; selected = self.selected_profession(); self.trades, self.source = payload
        self.jar = MinecraftJarData(self.owner.settings.minecraft_version); self.assets = AssetProvider(self.jar); self.trade_view.setItemDelegate(TradeDelegate(self.assets, self.trade_view)); self._portrait_cache.clear(); self._populate_professions(True); self._select_profession(selected); self.reload_button.setEnabled(True); self.loading.hide(); self.loading_label.setText("Installed trade-data check complete."); self.refresh()

    def _load_failed(self, message: str, _detail: str):
        self._load_job = None; self.reload_button.setEnabled(True); self.loading.hide(); self.loading_label.setText(f"Installed trade data could not be loaded; continuing with complete labeled reference data. {message}"); self.refresh()

    def _load_cancelled(self):
        self._load_job = None; self.reload_button.setEnabled(True); self.loading.hide(); self.loading_label.setText("Installed trade-data check cancelled; reference data remains available.")

    def reload(self): self._begin_load()
    ...
    def refresh(self):
        ...
        if profession: self.portrait.setPixmap(self._portrait_pixmap(profession))
        else: self.portrait.clear()
        exact = str(self.source).lower() == str(self.owner.settings.minecraft_version).lower(); self.source_label.setText(f"Trade data: {self.source}" + (" • selected-version source" if exact else " • labeled reference/fallback; matching installed trade JSON replaces this automatically when available"))
    ...
    def show_selected(self):
        ...
        if trade is None: return
        key = trade_key(trade); planned = self.uses.value(); max_uses = max(1, int(trade.max_uses or planned)); restocks = max(0, math.ceil(planned / max_uses) - 1)
        wants_count = _number(trade.wants); additional_count = _number(trade.additional_wants, 0) if trade.additional_wants else 0; gives_count = _number(trade.gives); emerald_flow = 0.0
        ...
        wants = trade.wants + (" + " + trade.additional_wants if trade.additional_wants else ""); self.want_text.setText(wants); self.give_text.setText(trade.gives)
        self.want_icon.setPixmap(self.assets.icon("minecraft:" + _item_token(trade.wants), 38).pixmap(QSize(38, 38))); self.give_icon.setPixmap(self.assets.icon("minecraft:" + _item_token(trade.gives), 38).pixmap(QSize(38, 38)))
        self.restocks.set_value(restocks); self.emeralds.set_value(f"{emerald_flow:+g}"); self.max_uses.set_value(trade.max_uses if trade.max_uses is not None else "—")
        details = trade.details or "No additional version-specific description is stored for this offer."; self.detail_note.set_text(f"Planning {planned} uses requires at least {restocks} restock(s) under the listed max-use value. Villager XP: {trade.xp if trade.xp is not None else 'not specified'}. {details}")
    ...
    def toggle_favorite(self):
        trade = self.selected_trade()
        if trade is None: return
        key = trade_key(trade); rows = list(self.owner.settings.favorite_trades); rows = [value for value in rows if value != key] if key in rows else rows + [key]
