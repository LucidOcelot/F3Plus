...
class LootWorkbenchDialog(_LootWorkbenchDialog):
    ...
    def __init__(self, *args, **kwargs):
        ...
        row = QHBoxLayout()
        row.addStretch()
        self.cancel_sim = QPushButton("Cancel simulation")
        self.cancel_sim.setEnabled(False)
        self.cancel_sim.clicked.connect(self._cancel_simulation)
        row.addWidget(self.cancel_sim)
        layout = self.layout()
    ...
    def _simulate_cancellable(self, table_id: str, pulls: int, seed: int, context: dict):
        ...
        hits = Counter(); totals = Counter(); examples = []
        for index in range(pulls):
            if index % 128 == 0:
                raise_if_cancelled()
            stacks = self.engine.roll(table_id, rng=rng, context=context)
            seen = set()
            for stack in stacks:
        ...
        raise_if_cancelled()
        possible = {row["item"] for row in self.engine.possible_items(table_id)}
    ...
    def run_sim(self, pulls: int):
        if self.engine is None or self._sim_job is not None:
            return
        item = self.tables.currentItem()
        if item is None:
            return
        table_id = item.data(0x0100)  # Qt.UserRole without importing another Qt symbol.
        seed = self.seed.value()
        context = {
            "killed_by_player": self.killed.isChecked(),
            "include_contextual_entries": self.contextual.isChecked(),
        }
        self._sim_context = (table_id, pulls, seed)
        self.summary.setText(f"Simulating {pulls:,} pulls in the background…")
        self.cancel_sim.setEnabled(True)
        self._sim_job = start_job(
            lambda: self._simulate_cancellable(table_id, pulls, seed, context),
            finished=self._simulation_finished,
            failed=self._simulation_failed,
            cancelled=self._simulation_cancelled,
        )

    def _simulation_finished(self, result):
        self._sim_job = None; self.cancel_sim.setEnabled(False)
        rows = result.get("stats", []); self.stats.setRowCount(len(rows))
        for r, row in enumerate(rows):
            first = QTableWidgetItem(self.icons.icon(row["item"], 24) if self.icons else None, str(row["item"]).removeprefix("minecraft:"))
            self.stats.setItem(r, 0, first)
            ...
            self.stats.setItem(r, 3, QTableWidgetItem(str(row["total_items"])))
    ...
    def _cancel_simulation(self):
        if self._sim_job is not None:
            self._sim_job.cancel(); self.cancel_sim.setEnabled(False); self.summary.setText("Cancelling simulation…")

    def closeEvent(self, event):
        if self._sim_job is not None:
            self._sim_job.cancel()
