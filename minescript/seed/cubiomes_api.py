...
STABLE_DEFAULT_ENUM = 27  # Explicit MC_1_21_3 in the bundled source.
...
def effective_enum(mc: int = 0) -> int:
    """Use the newest explicitly mapped stable release unless a caller chooses one."""
    return STABLE_DEFAULT_ENUM if int(mc) <= 0 else int(mc)


def biome_at(seed: int, x: int, y: int = 64, z: int = 0, *, dimension: int = 0, mc: int = 0) -> CubiomesBiomeResult:
    """Query bundled Cubiomes directly.

    ``mc=0`` uses F3+'s latest explicitly mapped stable Cubiomes release rather than
    the upstream MC_NEWEST alias, which can point at an unnamed/development ruleset.
    """
...
def structure_config(name: str, *, mc: int = 0) -> CubiomesStructureConfig:
    ...
    lib = _lib(); selected = effective_enum(mc)
