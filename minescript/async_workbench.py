...
class OperationDialog(_OperationDialog):
    ...
    def values(self):
        ...
        if self.location_panel is not None:
            for key in LOCATION_KEYS: values.pop(key, None)
    ...
    def _show_control_result(self, legacy, data): self.result_view.set_result(legacy, data, self.settings.theme, self.settings.custom_palette); self._show_page(1)
    ...
    def _run(self):
        ...
        except Exception as exc: QMessageBox.warning(self, legacy.name, str(exc)); return
        name = legacy.name
        if owner is not None:
            if self._run_control(owner, legacy, values): return
            try:
    ...
    def _job_finished(self, result):
        ...
        self.result_view.set_result(spec, result, self.settings.theme, self.settings.custom_palette); self._show_page(1)
