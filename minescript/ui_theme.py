...
def stylesheet(theme: str = "chorus", custom: dict | None = None) -> str:
    ...
    * {{ font-family: 'Segoe UI', 'Inter', 'SF Pro Text', sans-serif; font-size: 10.25pt; }}
    ...
    QFrame#InspectorHero {{ background: {p['surface2']}; border-left: 3px solid {p['primary']}; border-radius: {radius}px; }}
    ...
    QPushButton {{
        background: {p['surface2']}; border: 1px solid {p['border']}; border-radius: {radius}px;
        padding: 7px 12px; color: {p['text']}; font-weight: 650;
    }}
    ...
    QLineEdit, QSpinBox, QDoubleSpinBox, QComboBox, QPlainTextEdit, QTextBrowser, QTableWidget {{
        background: {p['surface2']}; color: {p['text']}; border: 1px solid {p['border']}; border-radius: {radius}px;
        padding: 7px; selection-background-color: {p['primary2']}; selection-color: {selected_text};
    }}
    ...
    QListWidget#NavList::item {{ padding: 10px 10px; margin: 2px 5px; border-radius: {radius}px; color: {p['muted']}; }}
    QListWidget#NavList::item:selected {{ background: {p['surface3']}; color: {p['text']}; border-left: 3px solid {p['primary']}; }}
    QListWidget#NavList::item:hover {{ background: {p['surface2']}; color: {p['text']}; }}

    QListWidget#ToolList {{ background: transparent; border: 0; outline: 0; }}
